...
# Downloaded from archive.mariadb.org: mirror.mariadb.org only serves the latest
# patch release of each series, so pinned URLs there start returning 404.
sudo mkdir mariadb_rpm
sudo chown airflow /mariadb_rpm

if [[ $(uname -p) == "aarch64" ]]; then
  wget https://archive.mariadb.org/mariadb-11.4.2/yum/fedora38-aarch64/rpms/MariaDB-common-11.4.2-1.fc38.$(uname -p).rpm -P /mariadb_rpm
  wget https://archive.mariadb.org/mariadb-11.4.2/yum/fedora38-aarch64/rpms/MariaDB-shared-11.4.2-1.fc38.$(uname -p).rpm -P /mariadb_rpm
  wget https://archive.mariadb.org/mariadb-11.4.2/yum/fedora38-aarch64/rpms/MariaDB-devel-11.4.2-1.fc38.$(uname -p).rpm -P /mariadb_rpm
else
  wget https://archive.mariadb.org/mariadb-11.4.2/yum/fedora38-amd64/rpms/MariaDB-common-11.4.2-1.fc38.$(uname -p).rpm -P /mariadb_rpm
  wget https://archive.mariadb.org/mariadb-11.4.2/yum/fedora38-amd64/rpms/MariaDB-shared-11.4.2-1.fc38.$(uname -p).rpm -P /mariadb_rpm
  wget https://archive.mariadb.org/mariadb-11.4.2/yum/fedora38-amd64/rpms/MariaDB-devel-11.4.2-1.fc38.$(uname -p).rpm -P /mariadb_rpm
fi
